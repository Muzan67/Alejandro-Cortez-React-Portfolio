import React from "react";

function Footer() {
  return (
    <div className="page-footer" id="footer" style={{textAlign:"center"}}>
        <a href="mailto: ac.tendo67@gmail.com">
        <img
          src="https://img.icons8.com/stickers/150/000000/gmail-new.png"
          alt="gmail-logo"
          class="icon"
        />
      </a>
      <a href="https://github.com/Muzan67">
        <img
          src="https://img.icons8.com/stickers/150/000000/github.png"
          alt="github-logo"
          class="icon"
        />
      </a>
      <a href="https://www.linkedin.com/in/alejandro-cortez1967/">
        <img
          src="https://img.icons8.com/stickers/150/000000/linkedin.png"
          alt="linkedin-logo"
          class="icon"
        />
      </a>
      <a href="https://docs.google.com/document/d/e/2PACX-1vTnvev0z6XZ_XMq-20eJFMywtdEi-carCY_vYhLQiEEXho8k5j3nQIsHsHDsetcNN8Qxl9rgDHAY5Nb/pub">
        <img
          src="https://img.icons8.com/stickers/150/000000/resume-website.png"
          alt="resume-logo"
          class="icon"
        />
      </a>
    </div>
  );
}

export default Footer;
